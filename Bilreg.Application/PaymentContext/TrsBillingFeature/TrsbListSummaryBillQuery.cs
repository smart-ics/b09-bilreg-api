...
//         var obat = listBill.Where(x => x.ModulGroup == BillModulGroup.Obat).ToList();
//         
...
//
//     private static List<TrsBillView> FilterUnprocessedJasa(IEnumerable<TrsBillView> bills, HashSet<string> processedIds) =>
//         bills.Where(x => x.ModulGroup == 0 && !processedIds.Contains(x.TrsBillingId)).ToList();
//
//     private static List<TrsbSummaryBillResponse> BuildResponse(decimal sumJasaHutang, decimal sumObatHutang,
//         List<TrsBillType> jasa, List<TrsBillType> obat, decimal admin, decimal materai)
//     {
//         static decimal SumJasa(List<TrsBillType> list, Func<TrsBillType, decimal> selector) => list.Sum(selector);
//         static decimal SumObat(List<TrsBillType> list, Func<TrsBillType, decimal> selector) => list.Sum(selector);
...
//             new(2, "Total Tagihan", SumJasa(jasa, x => x.Nilai.SubTotal), SumObat(obat, x => x.Nilai.SubTotal),
//                 Total(SumJasa(jasa, x => x.Nilai.SubTotal), SumObat(obat, x => x.Nilai.SubTotal))),
//             new(3, "Total Diskon", SumJasa(jasa, x => x.Nilai.Diskon), SumObat(obat, x => x.Nilai.Diskon),
//                 Total(SumJasa(jasa, x => x.Nilai.Diskon), SumObat(obat, x => x.Nilai.Diskon))),
//             new(4, "Total Pajak", SumJasa(jasa, x => x.Nilai.Tax), SumObat(obat, x => x.Nilai.Tax),
//                 Total(SumJasa(jasa, x => x.Nilai.Tax), SumObat(obat, x => x.Nilai.Tax))),
//             new(5, "Total Biaya", SumJasa(jasa, x => x.Nilai.Biaya), SumObat(obat, x => x.Nilai.Biaya),
//                 Total(SumJasa(jasa, x => x.Nilai.Biaya), SumObat(obat, x => x.Nilai.Biaya))),
