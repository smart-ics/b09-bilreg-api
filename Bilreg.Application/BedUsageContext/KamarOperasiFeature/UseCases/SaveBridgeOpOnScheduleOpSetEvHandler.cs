...
public class SaveBridgeOpOnScheduleOpSetEvHandler : INotificationHandler<OkScheduleOpSetEvent>
{
    ...
    public Task Handle(OkScheduleOpSetEvent notification, CancellationToken cancellationToken)
    {
        ...
        var bridgeOpCommand = new BridgeOperasiCmd(projectId, reg.RegId, notification.Aggregate.TglOp.ToString(DateFormatEnum.YMD),
            orderOp.TarifOperasi.TarifName, reg.Layanan.LayananId, reg.Layanan.LayananName, "0",
            reg.Polis.NoPolis, DateTimeOffset.UtcNow.ToUnixTimeMilliseconds());
