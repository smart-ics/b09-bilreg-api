...
public class RujukanDal : IRujukanDal
{
    ...
    public MayBe<RujukanType> GetData(IRujukanKey key)
    {
        const string sql = @"
                SELECT
                    aa.fs_kd_rujukan AS RujukanId, aa.fs_nm_rujukan AS RujukanName, 
	                aa.fb_aktif AS IsAktif, aa.fs_alm_rujukan AS AlamatRujukan1,
                    aa.fs_alm2_rujukan AS AlamatRujukan2, aa.fs_kota_rujukan AS KotaRujukan, 
	                aa.fs_kd_rujukan_tipe AS TipeRujukanId, 
	                aa.fs_kd_kelas_rs AS KelasRujukanId, aa.fs_kd_cara_masuk_dk AS CaraMasukDkId, 
                    ISNULL(bb.fs_nm_rujukan_tipe, '') TipeRujukanName,
                    ISNULL(cc.fs_nm_kelas_rs, '') KelasRujukanName,
                    ISNULL(cc.fn_nilai, 0) Nilai,
                    ISNULL(dd.fs_nm_cara_masuk_dk, '') CaraMasukDkName
                FROM 
                    ta_rujukan aa
                ...
                WHERE 
                    aa.fs_kd_rujukan = @RujukanId";

        var dp = new DynamicParameters();
        dp.AddParam("@RujukanId", key.RujukanId, SqlDbType.VarChar);

        using var conn = new SqlConnection(ConnStringHelper.Get(_opt));
        return MayBe
            .From(conn.ReadSingle<RujukanDto>(sql, dp))
            .Map(x => x.ToModel());
    }

    public MayBe<IEnumerable<RujukanType>> ListData()
    {
        const string sql = @"
                SELECT
                    aa.fs_kd_rujukan AS RujukanId, aa.fs_nm_rujukan AS RujukanName, 
...
{
    public string RujukanId { get; set; }
    public string RujukanName { get; set; }
    public bool IsAktif { get; set; }
    public string AlamatRujukan1 { get; set; }
    public string AlamatRujukan2 { get; set; }
    public string KotaRujukan { get; set; }
    public string TipeRujukanId { get; set; }
    public string TipeRujukanName { get; set; }
    public string KelasRujukanId { get; set; }
    public string KelasRujukanName { get; set; }
    public string CaraMasukDkId { get; set; }
    public string CaraMasukDkName { get; set; }
    public decimal Nilai { get; set; }

    public RujukanType ToModel()
    {
        var alamat = new AlamatType([AlamatRujukan1, AlamatRujukan2], KotaRujukan, "-");

        var tipeRujukan = new TipeRujukanType(TipeRujukanId, TipeRujukanName);
        var kelasRujukan = new KelasRujukanReff(KelasRujukanId, KelasRujukanName);
        var caraMasuk = CaraMasukDkType.Default with { CaraMasukDkId = CaraMasukDkId, CaraMasukDkName = CaraMasukDkName };
        return new RujukanType(RujukanId, RujukanName, IsAktif, alamat, tipeRujukan, kelasRujukan, caraMasuk);
    }
