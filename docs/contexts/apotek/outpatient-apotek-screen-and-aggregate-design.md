...
| `Invoice` | Owns one medication sale, catalog sales items including BHP, item-level charges, invoice-level charges, pricing snapshot, payer, financial disposition, and commercial adjustments on the Invoice. Content remains mutable while Tata Rekening still permits modification. Does not own Credit Note, Refund, or Financial Adjustment documents. Optional Tata Rekening correction correlation is allowed. | Sales Order quantity authority, payment evidence, stock, physical dispensing, Tata Rekening permission rules, Financial Clearance, Credit Note lifecycle, or a separate invoice-component model |
