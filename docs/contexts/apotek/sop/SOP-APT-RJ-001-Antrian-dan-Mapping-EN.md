...
| Pharmacy System | Subsystem | Attempts Tracker Mapping to existing Resep, records Outpatient Queue Mapping, and displays per-source progress. |
...
4. **Pharmacy System** records a separate Outpatient Queue Mapping for every resolved Resep and displays each mapped Resep with its authoritative progress.
...
6. **Patient or Caregiver** presents the Queue Number and available Resep, registration, or direct-request evidence.
7. **Pharmacy Staff** selects the unresolved Queue Entry and identifies every applicable existing Resep or Sales Order.
8. When a Resep Fisik is presented, **Pharmacy Staff** records it before requesting Telaah Resep; when a Direct Medication Request is presented, **Pharmacy Staff** evaluates it under the acceptance procedure.
...
- **Pharmacy Staff** selects the correct Resep or medication-demand source.
