...
| Pharmacy Staff | Human | Verifies separate mappings and progress, prepares each cleared Dispensing separately, coordinates payer readiness, communicates exceptions, and performs one pickup call. |
...
| Pharmacy System | Subsystem | Projects per-demand progress and records separate Invoices, Dispensings, dispense, and handover outcomes. |
...
2. One Pharmacy Queue Entry has separate Outpatient Queue Mappings to at least two Resep Kerja and/or Jual Bebas.
3. Each Resep has its own Telaah Resep.
4. Each accepted source has its own Sales Order and active primary outpatient Dispensing.

## 4. Operational Steps

1. **Pharmacy Staff** opens the common Queue Entry in `Apotek Rajal`.
2. **Pharmacy System** displays every mapped demand separately with its source, Sales Order, payer, Sales Order Items, Invoice, Dispense Authorized evaluation, and Dispensing progress.
3. **Pharmacy Staff** verifies that no demand, Sales Order, Invoice, or Dispensing has been merged with another demand.
...
6. **Pharmacy Staff** prepares each released Dispensing separately and records completion for each one.
...
15. **Pharmacy System** records Medication Dispense and Medication Handover against every applicable Dispensing and Sales Order separately.
16. **Inventory** records separate Remove Stock or return Mutasi outcomes for each originating Dispensing.
...
- **Pharmacy System** retains the exception under its originating Sales Order and Dispensing.

### 5.3 One Final Dispense Review fails

- **Pharmacist** records the failure reason and affected quantity for the originating Dispensing and does not authorize its handover.
- **Pharmacy System** appends an immutable review record with the Pharmacist and effective business time and returns only that Dispensing from `Prepared` to `Preparing`; other demand records remain unchanged.
...
2. Every Resep, Sales Order, Invoice, and Dispensing retains independent identity and progress.
