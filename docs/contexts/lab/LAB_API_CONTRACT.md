...
## Lab result (server-owned structure — Phase 4)

Result **structure** is generated by the server from immutable `LabOrderItemComponent` snapshots at record time. The client submits **values only** keyed by `componentId` (`MLCxxxx`). There is **no** client structure merge, reconciliation, or V2 endpoint.

| Route | Notes |
|-------|-------|
| `GET .../LabResultFeature/{orderId}` | Read current result document + persisted snapshot lines |
| `POST .../LabResultFeature/record` | Values-only record (see below) |
| `PATCH .../LabResultFeature/verify` | Pathologist verification |
| `PATCH .../LabResultFeature/amend` | New version; server regenerates structure from order snapshots |
| `GET .../LabResultFeature/worklist/verification` | Verification worklist |
| `GET .../LabResultFeature/{orderId}/pdf` | Operational PDF |

### Record result (values only)

| | |
|--|--|
| **Route** | `POST /api/LabContext/LabResultFeature/record` |
| **Body** | `LabResultRecordCmd` |

**Request fields:**

| Field | Type | Required | Notes |
|-------|------|----------|-------|
| `orderId` | string | yes | Internal LWF order id (lab UI) |
| `userId` | string | yes | Recording user |
| `resultSource` | int | yes | `LabResultSourceEnum` |
| `values` | `LabResultRecordValueDto[]` | yes | One entry per submitted value |

**Value line (`LabResultRecordValueDto`):**

| Field | Type | Required | Notes |
|-------|------|----------|-------|
| `componentId` | string | yes | Must match a line on the server scaffold (`MLCxxxx`) |
| `value` | string | yes | Interpreted by server from snapshotted `resultType` on the order component |

**Client must not send:** `componentCode`, `componentName`, `resultType`, `unit`, `referenceRangeText`, ordering, or arbitrary extra lines.

**Server behaviour:** Build scaffold from `LabOrderItemComponent` → validate values → persist full `LabResultItem` snapshot (structure + values) on `LabResultDocument`.

**Validation error codes:**

| Code | When |
|------|------|
| `LAB_RESULT_UNKNOWN_LINE` | `componentId` not on server scaffold |
| `LAB_RESULT_EXTRA_LINE` | Duplicate `componentId` or too many value lines vs scaffold |
| `LAB_MANDATORY_COMPONENT_MISSING` | Mandatory scaffold line has no value |
| `LAB_RESULT_SCAFFOLD_EMPTY` | Order has no component snapshots |

### Get result

**Item fields (persisted snapshot):** `itemNo`, `componentId`, `testId` (`LTDxxxx`), `testName`, `componentCode`, `componentName`, `sequenceNo`, `resultType`, value fields (`numericValue`, `textValue`, `optionValue`, `narrativeValue`), `unit`, `referenceRangeText`, `isMandatory`, `flagStatus`.

### Amend result

| | |
|--|--|
| **Route** | `PATCH /api/LabContext/LabResultFeature/amend` |
| **Body** | `orderId`, `reason`, `amendedBy` |

Creates a new result **version** with structure **regenerated** from order component snapshots (empty values; re-record required). Prior verified version is retained immutably (`isCurrentVersion = false`).
