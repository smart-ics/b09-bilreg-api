...
- CPOE memiliki Clinical Order intent, authorization, routing, lifecycle, Order Occurrence, final disposition, serta Fulfilment Evidence. RNA menerima pekerjaan per `OrderOccurrenceId`, memiliki bukti pelaksanaan, dan mempertahankan `ClinicalOrderId` sebagai korelasi induk.
...
- `docs/contexts/cpoe/CPOE-DOMAIN.md` digunakan hanya untuk batas Clinical Order, receiver coordination, final disposition, Order Occurrence, Fulfilment Evidence, dan reconciliation. Correction notice dari RNA tidak mengubah terminal status Order Occurrence secara otomatis.
...
| [SOP-RNA-S01 — Penyelesaian Order Occurrence oleh RUANG RANAP](SOP-RNA-S01-Pelaksanaan-RNA-Service-Berdasarkan-Clinical-Order.md) | Receiver dan Performer RUANG RANAP | Order Occurrence berstatus `Active` diarahkan ke RNA | Satu occurrence berakhir `Completed` atau `Not Performed`, atau pekerjaan aktif menjadi source-cancelled; occurrence lain pada Clinical Order yang sama tetap independen | CPOE Order Fulfilment Tracking, Scheduled Order Management, dan Order Cancellation | Upstream: CPOE dan Tarif; downstream: S04; bila bukti salah: S03 |
...
| [SOP-RNA-S03 — Koreksi Fakta Pelaksanaan Layanan RUANG RANAP](SOP-RNA-S03-Koreksi-RNA-Service-Execution.md) | Kepala Ruangan atau Reviewer Independen sesuai materialitas | Kesalahan fakta pelaksanaan teridentifikasi | Fakta asli dipertahankan; koreksi dicatat append-only; bila terkait native CPOE, correction notice menambah accountable history dan conflict/reconciliation need tanpa mengubah terminal status Order Occurrence | RNA Execution Correction; CPOE accountable reconciliation | Upstream: S01/S02; downstream: CPOE dan/atau S04 |
