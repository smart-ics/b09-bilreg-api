...
|---|---|---|---|---|
| P1S1 | GO | 2026-09-17: Added the three assessment columns and the visit/triage indexes with additive defaults; preserved existing rows. | Review 1: DDL and backward-compatibility checks passed. Review 2: Unique-key and filtered-index behavior confirmed. Final: GO. | — |
| P1S2 | GO | 2026-09-17: Created the triage concept-map table, composite key, concept index, and SQL project registration. | Review 1: Schema matched the approved model. Review 2: Master-data ownership and migration scope confirmed. Final: GO. | — |
| P1S3 | GO | 2026-09-17: Seeded the dedicated IGD Triage Paper, sections, custom sections, and referenced concepts idempotently. | Review 1: Seed structure and paper isolation checked. Review 2: All required triage concepts were traced to the paper. Final: GO. | — |
| P1S4 | GO | 2026-09-17: Added idempotent mappings for all triage fields, values, GCS total, ATS, colors, and manual black override. | Review 1: Mapping coverage reviewed against the architecture matrix. Review 2: Concept references and fail-closed inputs confirmed. Final: GO. | — |
| P2S1 | GO | 2026-09-17: Extended the assessment model with IGD linkage fields, pending-registration status, key value object, and invariants. | Review 1: Domain types and enum values matched the contract. Review 2: INV-A1 through INV-A6 tests passed. Final: GO. | — |
| P2S2 | GO | 2026-09-17: Added triage concept-map domain/DAL support and extended assessment queries with projections and pending-row quarantine. | Review 1: DAL boundaries and column projections checked. Review 2: Registration/patient queries exclude pending assessments as required. Final: GO. | — |
| P2S3 | GO | 2026-09-17: Implemented the IGD triage assessment generation command with idempotency, mapping resolution, pending registration, and fail-closed handling. | Review 1: Normative generation flow reviewed. Review 2: Success, duplicate, missing-map, and missing-paper tests passed. Final: GO. | — |
| P2S4 | GO | 2026-09-18: Implemented visit-based assessment linking with all-or-nothing registration keys, per-row scopes, and partial-failure reporting. | Review 1: Link algorithm and key population verified. Review 2: Idempotent empty and repeated-link scenarios passed. Final: GO. | — |
| P2S5 | GO | 2026-09-18: Added by-visit and pending-registration queries with monitoring projections, labels, and source information. | Review 1: Pending visibility and quarantine rules checked. Review 2: Oldest-first ordering and read-only behavior confirmed. Final: GO. | — |
| P2S6 | GO | 2026-09-18: Wired the generate, link, by-visit, and pending-registration endpoints with authorization and standard JSend responses. | Review 1: Routes and use-case wiring matched the API contract. Review 2: Authorization scope and legacy action behavior confirmed. Final: GO. | — |
| P3S1 | GO | 2026-09-18: Created the operational SMASS task table with business-key uniqueness, visit lookup, and processable-status indexes. | Review 1: DDL and index design reviewed against task invariants. Review 2: No backfill, soft delete, or audit columns confirmed. Final: GO. | — |
| P3S2 | GO | 2026-09-18: Implemented the SMASS task aggregate, enums, key, state transitions, retry guard, and error truncation rules. | Review 1: State machine reviewed against INV-T1 through INV-T8. Review 2: Domain transition and retry tests passed. Final: GO. | — |
| P3S3 | GO | 2026-09-18: Implemented task DAL/repository upsert, business-key lookup, visit listing, processable listing, and round-trip persistence. | Review 1: Repository boundary and Scrutor registration checked. Review 2: Insert, update, load, and ordering tests passed. Final: GO. | — |
| P4S1 | PLANNED | — | — | — |
| P4S2 | PLANNED | — | — | — |
| P4S3 | PLANNED | — | — | — |
| P4S4 | PLANNED | — | — | — |
